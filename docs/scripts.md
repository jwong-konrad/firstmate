...
| `fm-upstream-pin.sh`     | Own the `.upstream-pin` format, the bounded read-only upstream fetch, and the unreviewed-commit count |
| `fm-upstream-gate.sh`    | Refuse a push or PR to main that ingests upstream commits the vetting pin does not cover |
...
| `fm-captain-idle-handoff.sh` | Capture a handoff and remind the captain to clear after a long quiet stretch or a session close to auto-compacting (docs/captain-idle-handoff.md) |
| `fm-context-fill-lib.sh` | Shared context-fill, window, and compaction-point reader for the auto-handoff's context-fill trigger (docs/captain-idle-handoff.md) |
| `fm-banner-lib.sh`       | Shared attention-banner shape used by the guards, the read-only notice, and the idle reminder |
| `fm-turnend-guard-grok.sh` | Grok Stop-hook adapter for the primary turn-end guard                              |
| `fm-arm-pretool-check.sh` | Stable PreToolUse transport for the watcher-arm command policy (docs/arm-pretool-check.md) |
| `fm-arm-command-policy.mjs` | Semantic owner of the watcher-arm PreToolUse policy (docs/arm-pretool-check.md)   |
| `fm-continuity-pretool-check.sh` | Narrow Claude recovery gate when progressing work has no live and no recently-servicing watcher (docs/watcher-continuity.md) |
...
| `fm-pr-target-guard.sh`  | Refuse work in a checkout whose recorded pull-request target is a repository this home must never open a pull request against (docs/configuration.md) |
| `fm-env-clean.sh`        | Run a spawned worker's launch command under a deny-by-default environment (docs/worker-environment.md) |
...
| `fm-auto-afk.sh`         | Ask firstmate to enter away mode when the captain has been quiet past the configured stretch and work is under way (docs/captain-idle-handoff.md) |
...
| `fm-supervision-lib.sh`  | Shared progressing-work-without-fresh-watcher-beacon predicate                       |
| `fm-progress-lib.sh`     | Shared progressing-task predicate and its durable per-task verdict records            |
