...
The session-start secondmate liveness sweep, the digest's per-task `agent:` line, `fm-crew-state.sh`, and `fm-send.sh`'s text-send refusal all use a deeper `fm_backend_agent_liveness` probe where verified.
Today that probe can classify tmux and herdr endpoints as `alive`, `shell` (endpoint present, agent exited), `no-endpoint`, or `unknown`; zellij, Orca, and cmux report `unknown` until their own agent-process classifiers are verified, and an inconclusive read is never treated as alive.
...
The boundary described here is a filesystem-write boundary only: the worker's launch environment is a separate boundary that neither the sandbox nor the override contract governs, owned by "Worker launch environment" below and by [`worker-environment.md`](worker-environment.md).
...
FM_PROGRESS_RECORD_TTL=86400   # absolute expiry for any state/.progress-<id> verdict record; with idle bounded above, this backstops `progressing` records and unusable timestamps (docs/supervision-arming.md)
...
FM_CONTINUITY_SERVICE_GRACE=300   # defaults to FM_GUARD_GRACE; seconds the Claude continuity gate keeps allowing fleet commands after a watcher cycle delivered an actionable wake but left no lock holder (docs/supervision-deadlock-guard.md)
FM_SIGNAL_GRACE=30      # seconds to coalesce nearby status and turn-end signals into one wake
FM_CAPTAIN_RE='done:|needs-decision:|blocked:|failed:|PR ready|checks green|ready in branch|merged'   # captain-relevant status regex; nonterminal progress verbs remain excluded even when their prose matches
FM_CLASSIFY_PAUSED_VERB=paused     # leading status verb for a declared external wait; excluded from FM_CAPTAIN_RE and distinct from blocked
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; stale panes whose crew is not provably working surface immediately unless the task is awaiting firstmate (declared pause, captain-held transfer, or an open keyed needs-decision/blocked)
FM_PAUSE_RESURFACE_SECS=3600       # seconds before an idle task awaiting firstmate re-surfaces for a recheck in the watcher or away-mode daemon, and the cadence for the watcher's unchanged-state wedge recheck
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive provably-working stale escalations on the same pane before demand-deep-inspection is added; an escalation requires the task's reconciled state to have changed since the previous one
...
FM_SEND_LIVENESS_GRACE= # seconds between fm-send's agent-liveness re-probes before it refuses a confirmed-dead endpoint; defaults to FM_SEND_SLEEP, 0 re-probes with no wait
FM_SEND_LIVENESS_ATTEMPTS= # how many of those re-probes fm-send pays; defaults to FM_SEND_RETRIES, 0 takes the first verdict as final
...
`fm-teardown.sh` retries exactly two transient return-failure signatures, each up to `FM_TREEHOUSE_RETURN_LOCK_RETRIES` times through the same bounded loop: Git's `Unable to create '...index.lock': File exists`, and the empty-stderr git-reset race (`failed to return worktree: git reset --hard <ref>: ` with nothing after the trailing colon); any other failure, including a reset failure carrying git's own diagnostic text, aborts immediately (see the [`bin/fm-teardown.sh`](../bin/fm-teardown.sh) header).
...
The reset-race signature has no lock file, so when its retries exhaust teardown simply refuses loudly with no stale-cleanup step.
