...
FM_PROGRESS_RECORD_TTL=86400   # absolute expiry for a state/.progress-<id> verdict record; every real resumption path invalidates it sooner (docs/supervision-arming.md)
...
FM_PROGRESS_RECORD_TTL=86400   # seconds a durable state/.progress-<id> verdict stays usable before it is re-reconciled; changed task evidence invalidates it sooner (docs/supervision-arming.md)
