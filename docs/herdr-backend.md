...
Rather than add a second herdr classifier, `fm_backend_herdr_agent_liveness` (`bin/backends/herdr.sh`) is a thin wrapper around the already-verified `fm_backend_herdr_pane_agent_state`: `no-agent` maps to `shell` (a restored, agent-less bare shell), `dead` maps to `no-endpoint` (a structurally-gone pane), `live` maps to `alive`, and `unknown` stays `unknown`.
`fm_backend_herdr_agent_alive` remains the coarse projection the sweep reads, where `shell` and `no-endpoint` both collapse to its `dead` verdict - neither is a live secondmate; the next section owns why the finer split exists.
No new empirical verification was needed for the mapping itself - `fm_backend_herdr_pane_agent_state`'s four states are already verified above (both at the unit level and, for `no-agent`, against the real binary via the respawn-idempotency e2e test); this wrapper only renames them for the generic `fm_backend_agent_liveness` dispatcher (`bin/fm-backend.sh`) that also serves the tmux adapter (`docs/tmux-backend.md` "Agent liveness probe").
