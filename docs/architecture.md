...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or an X-mode mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, tasks awaiting firstmate that remain idle past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
Repeated provably-working stale escalations on the same pane add an escalation count to the wake reason and, at `FM_WEDGE_DEMAND_INSPECT_COUNT`, a `demand-deep-inspection` marker.
An escalation requires the task's reconciled state to differ from the state recorded at the previous one, because the count is the urgency signal and bumping it without new evidence is misinformation; an unchanged state gets a bounded `FM_PAUSE_RESURFACE_SECS` recheck that is deliberately not counted as an escalation.
...
A task that is awaiting firstmate - a declared `paused:` external wait, a verified captain-held transfer, an open keyed `needs-decision:`/`blocked:` in the durable status fold, or a finished ship task parked on an armed unmerged PR - is separately absorbed while idle and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge, because its idle pane is the expected state and the decision was already surfaced when its status line was written.
`bin/fm-classify-lib.sh` owns the status-declared part of that predicate; the pending-merge part is read from this home's own task state by `bin/fm-watch.sh` and stays watcher-local, because no shared contract models a merge gate today (`docs/supervision-arming.md`).
Run-step reconciliation still overrides both: a task that raised a decision and then started a run, or that firstmate steered again after its `done:` line, counts as working.
...
An open keyed decision deliberately requires no such liveness proof, because a crew that raised a decision and then stopped keeps a live pane by design, which is exactly the population being absorbed.
...
A task awaiting firstmate trades that silence for one bounded recheck per pause window, so a forgotten pause or an undecided open decision cannot remain invisible indefinitely.
...
Only when no matching run exists does it fall back to an agent-liveness read, then the pane busy-signature, and then a status-log event whose verb maps to a recognized run-state; a dead pane without a run reports unknown instead of trusting a stale log, and so does a pane that still exists but holds no agent, reported as `source: endpoint` so the bare shell an exited agent leaves is diagnosable rather than passing as healthy.
...
Arming is conditional: it declines with one `watcher: not armed - nothing to watch` line when no task is progressing and no poll is armed, because a fleet that is entirely parked, awaiting firstmate, failed, or gone changes only when firstmate acts.
`--force` overrides that gate for a deliberate arm, and is a flag rather than an env prefix because `bin/fm-arm-pretool-check.sh` denies an env-prefixed arm as a wrapper.
`bin/fm-progress-lib.sh` owns that predicate, drives it from run-step reconciliation rather than pane idleness, and is shared with the turn-end, fleet, and continuity guards so all four agree on what counts as work; [supervision-arming.md](supervision-arming.md) records the design and the seams left to the deadlock and tripwire tasks.
...
Claude keeps its tracked background-task protocol and adds a narrow PreToolUse continuity gate that allows drain, arm recovery, and fail-closed teardown while refusing only other fleet commands when work is progressing and supervision is absent - neither a live identity-matched lock holder nor a recent watcher cycle that delivered an actionable wake ([`supervision-deadlock-guard.md`](supervision-deadlock-guard.md)).
...
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, or if work is progressing and that watcher stops running or queued wakes are waiting to be drained.
...
On every verified primary harness, tracked hook integration gives the primary session a push-based backstop: when work is progressing and no identity-matched watcher lock with a fresh beacon is live, direct Stop hooks block and passive turn-end hooks force one bounded follow-up.
...
The deeper agent-process liveness probe is separate from that busy-state poll: tmux and herdr have verified classifiers, while zellij, Orca, and cmux currently report `unknown` rather than guess.
`bin/fm-backend.sh`'s `fm_backend_agent_liveness` owns that classification as `alive`, `shell` (the endpoint exists but its agent has exited, leaving a bare shell that would execute a steer instead of receiving it), `no-endpoint`, or `unknown`, and serves the secondmate respawn sweep, the session-start digest's per-task `agent:` line, `fm-crew-state.sh`'s no-run fallback, and `fm-send.sh`'s text-send refusal; `fm_backend_agent_alive` remains its coarse `alive`/`dead`/`unknown` projection for existing callers.
...
Clones with tracked modifications, non-default branches, detached HEADs with unique commits, diverged defaults, and default branches checked out in another worktree are reported as `STUCK:` with their behind count and left untouched.
Untracked-only dirt blocks the fast-forward only when the incoming changes would create or modify one of the untracked paths; that collision is reported as its own quantified `STUCK:` naming the colliding paths, while non-colliding untracked files never block the sync (`bin/fm-fleet-sync.sh`'s header owns the exact rules).
