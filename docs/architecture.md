...
A task that is awaiting firstmate - a declared `paused:` external wait, a verified captain-held transfer, or an open keyed `needs-decision:`/`blocked:` in the durable status fold - is separately absorbed while idle and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge, because its idle pane is the expected state and the decision was already surfaced when its status line was written.
`bin/fm-classify-lib.sh` owns that awaiting-firstmate predicate, and run-step reconciliation still overrides it: a task that raised a decision and then started a run counts as working.
