...
It runs only in a primary home, and it denies only an executed `bin/fm-*.sh` command other than `bin/fm-wake-drain.sh`, `bin/fm-watch-arm.sh`, or the ordinary literal `bin/fm-teardown.sh`, and only when work is progressing and supervision is absent.
[`watcher-continuity.md`](watcher-continuity.md) owns that liveness contract, including the three-state live/serviced/absent test and the progressing predicate behind it.
Ordinary shell commands, fleet-script names used as data, all commands in a fleet with nothing progressing, child worktrees, wake drain, watcher arm, and ordinary literal teardown remain allowed.
