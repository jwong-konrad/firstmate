...
This fork carries only the remote job execution core so far: `fm-remote-job-lib.sh`, `fm-remote-job-worker.sh`, `fm-remote-job-reap-orphans.sh`, `fm-remote-entrypoint.sh`, `fm-remote-delta-read.sh`, `fm-remote-file.sh`, and `fm-remote-readiness-lib.sh`.
The remote second-mate provisioning and control layer this page also describes - `fm-on.sh`, `fm-remote-doctor.sh`, `fm-remote-secondmate-control.sh`, `fm-remote-home-seed.sh`, `fm-remote-home-provision.sh`, `fm-remote-inherit.sh`, `fm-remote-inherit-push.sh`, and the Herdr guard pair - is not present here yet.
Until it lands, treat the provisioning and routing sections below as the contract those scripts implement rather than as commands this fork can run.
