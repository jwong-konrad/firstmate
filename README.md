...
| `/handoff`         | Run the `/stow` sweep plus capture the session's reasoning - decisions with rationale, open threads, and next steps - to a handoff document under `data/handoffs/`, so a session can be cleared and picked up at the same depth |
...
- [docs/worker-environment.md](docs/worker-environment.md) - what environment a spawned worker does and does not inherit from you, and the residual risks the allowlist does not close.
...
- [docs/captain-idle-handoff.md](docs/captain-idle-handoff.md) - why a long quiet stretch triggers an automatic handoff capture and a "clear before session" reminder, which idleness signal it measures, and the default threshold.
