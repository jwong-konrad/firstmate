...
Shared tracked material is `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.upstream-pin`, `.github/workflows/`, `bin/`, `.agents/skills/`, and public `skills/`.
...
.upstream-pin        tracked record of how far the pull-only upstream template has been reviewed; committed. Its own comments and bin/fm-upstream-pin.sh own the format; bin/fm-upstream-gate.sh enforces it in CI and bootstrap reports drift from it
...
config/spawn-env-allow  optional widening of the worker launch-environment allowlist; LOCAL, gitignored; one exact variable NAME per line; absent means the built-in allowlist alone (section 4, docs/configuration.md). NOT inherited into secondmate homes
config/project-env  optional per-project worker environment; LOCAL, gitignored; "<project> KEY=VALUE ..." keyed by the projects/<name> clone basename, delivered through fm-spawn's injection seam (section 4, docs/configuration.md). Never applies to a secondmate spawn; NOT inherited into secondmate homes
config/pr-target-deny  optional list of repositories this home must never open a pull request against; LOCAL, gitignored; one full URL, host/owner/name, or bare owner/name per line, enforced by bin/fm-pr-target-guard.sh at every non-secondmate spawn and as a precondition of the no-mistakes ship brief (docs/configuration.md). Inherited by secondmate homes
config/app-checkouts  optional map of a project to an absolute app-source checkout outside this home, fast-forward-pulled at spawn; LOCAL, gitignored; fail-open, kill switch FM_NO_CHECKOUT_PULL=1, bounded by FM_APP_PULL_TIMEOUT, and a path firstmate itself owns is refused rather than pulled (section 4, docs/configuration.md). Never applies to a secondmate spawn; NOT inherited into secondmate homes
...
5. **Fleet-state digest** - the compact backlog listing owned by `bin/fm-session-start.sh`; every `state/<id>.meta`; a bounded tail of each task's `state/<id>.status` (labeled as wake-EVENT history, not current state, with the full log path printed for a deeper read); the `state/.afk` flag; and two cheap reads of each task's recorded backend endpoint, printed as separate `endpoint:` and `agent:` lines.
   Those two lines answer different questions and must not be conflated: `endpoint:` is presence only, while `agent:` reports whether a worker process is actually running in it, because an exited agent leaves a present endpoint holding a bare shell that would execute anything sent to it rather than receive it.
   An `agent:` line reporting the classification as inconclusive is never a live worker; treat it as unknown, not healthy.
   It is also never a confirmed-gone worker: a backend this session cannot read reports the `agent:` line as inconclusive rather than asserting an endpoint is gone, even when the `endpoint:` line reads dead for the same unreadable backend.
   Neither line is a full state read - when you need a crew's actual current state (a run-step, not just "is something there"), read it with `bin/fm-crew-state.sh <id>` as before; the digest deliberately skips that deeper, slower read for every task so it stays fast and bounded.
...
For an ordinary direct report whose endpoint is dead, whose endpoint is present but running no worker, or whose metadata has no window, load `stuck-crewmate-recovery` and preserve the recorded worktree and unlanded work while reconciling ownership.
...
The spawn must resolve a genuine isolated task worktree distinct from the primary checkout and from every other in-flight task's recorded worktree; a failed isolation assertion, a double-allocation refusal, or a denied pull-request target stops the task.
A double-allocation refusal names the colliding task id: inspect that task and tear it down properly if it is finished or dead, or get captain guidance, before retrying the spawn.
