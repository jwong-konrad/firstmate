...
# One case, not many: every phase below shares a single live worker and one
# state root, and deliberately mutates and re-probes them in order. run_case
# dispatches each case in a subshell, so splitting these phases would destroy
# exactly the shared live fixture they exist to exercise.
test_remote_job_queue_and_worker_contract() {
  # The file-scope trap cannot see the auxiliary PIDs this case assigns, so
  # re-arm the same cleanup here, where they are in scope. The file-scope trap
  # stays as the backstop: it reads worker.pid from disk and removes TMP_ROOT.
  trap cleanup_remote_job_fixture EXIT
...
run_case test_remote_job_queue_and_worker_contract
fm_case_summary "fm-remote-job"
