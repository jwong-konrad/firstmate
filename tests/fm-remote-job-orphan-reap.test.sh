...
# This repo's fm_test_tmproot registers cleanup in a shell array, so the
# registration dies with the command-substitution subshell that made the
# directory and takes the directory with it. Recreating it here is the same
# compensation tests/fm-remote-job.test.sh already makes.
mkdir -p "$TMP_ROOT"
TMP_ROOT=$(cd "$TMP_ROOT" && pwd -P)
REAPER="$ROOT/bin/fm-remote-job-reap-orphans.sh"

# The live-pid safety net is file-backed, not an array: run_case dispatches each
# case in a subshell, so a PID tracked inside a case would never reach a parent
# array and a failing case would leak a real worker whose supervisor respawns it.
TRACKED_PID_FILE="$TMP_ROOT/tracked-pids"
: > "$TRACKED_PID_FILE"
orphan_cleanup() {
  local pid
  while IFS= read -r pid; do
    ...
    kill -KILL "$pid" 2>/dev/null || true
  done < "$TRACKED_PID_FILE"
  fm_test_cleanup
  # fm_test_cleanup cannot see this root for the reason above, and this suite
  # leaves live-process fixtures and git repos behind, so remove it explicitly.
  rm -rf "$TMP_ROOT"
}
...
# Each case keeps its whole live-process sequence inside one function: the
# phases share live PIDs in shell variables, which per-phase subshells would
# destroy. The two cases are independent of each other, so they dispatch
# through run_case separately.
test_reaper_spares_a_live_code_root_and_stops_a_pruned_one() {
...
test_reaper_sweeps_an_already_orphaned_worker() {
...
run_case test_reaper_spares_a_live_code_root_and_stops_a_pruned_one
run_case test_reaper_sweeps_an_already_orphaned_worker
fm_case_summary "fm-remote-job-orphan-reap"
