...
- `FLEET_SYNC: <repo>: STUCK: on <state>, N commits behind <base> - needs attention` - the clone has tracked modifications, is on a non-default branch, detached with unique commits, or diverged, so the sync left it untouched (never forcing or discarding); it will keep falling behind until you look.
- `FLEET_SYNC: <repo>: STUCK: N untracked path(s) collide with incoming changes (<paths>) - needs attention` - the clone's only dirt is untracked, but the pending fast-forward would create or modify the named paths, so the sync refused rather than risk that data; inspect the named untracked paths and move them aside (or confirm they are disposable) before the next refresh.
  Untracked-only dirt that no incoming path touches does not STUCK; it fast-forwards normally (`bin/fm-fleet-sync.sh`'s header owns the exact rules).
